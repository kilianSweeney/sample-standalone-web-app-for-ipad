$(function(){
    ...
    var questions = [
        ...
        listBuild = function(){
          //this function not used unless generating a list of products and logic for client review
          var updateTable = function(prop,name){
            var obj = {name: name, items:[],itemNames:[]},
            tmp,tmpStr,
                fixName = function(old){
                  var newT = old;
                  switch(old){
                    case 'IPAC':
                      newT = 'CAPXL';
            ...
            for(p in products){
              tmp = products[p];
              if(prop === 'loop' || prop === 'accessories'){
                  obj.items.push(tmp[prop]);
              }
              else {
                for(a in tmp[prop]){
                  if(obj.itemNames.indexOf(tmp[prop][a].name) === -1){
                    obj.items.push({name:tmp[prop][a].name,prods:[tmp[prop][a].value]});
                    obj.itemNames.push(tmp[prop][a].name);
                  }
                  else {
                    for(i in obj.items){
                      if(obj.items[i].name === tmp[prop][a].name)obj.items[i].prods.push(tmp[prop][a].value);
                    }
                  }
                }
              }
              if(prop === 'appType')$('#list thead tr').append('<th>' + fixName(tmp.id) + '</th>');
            }
            if(prop === 'loop' || prop === 'accessories'){
              tmpStr = '<tr><td>' + obj.name +'</td><td>&nbsp;</td>'; 
              for(i in obj.items){
                tmpStr += '<td>' + obj.items[i] + '</td>'
              }
              tmpStr += '</tr>';
              $('#list tbody').append(tmpStr);
            }
            else {
              for(i in obj.items){
                tmpStr = '<tr><td>' + obj.name +'</td><td>' + obj.items[i].name + '</td>';
                for(p in obj.items[i].prods){
                  tmpStr += '<td>' + obj.items[i].prods[p] + '</td>';
                }
                tmpStr += '</tr>';
                $('#list tbody').append(tmpStr);
              }
          ...
          updateTable('appType','Application');
          updateTable('entranceType','Type of Gate');
          updateTable('userNum','Number of Employees/Residents');
          updateTable('entranceNum','Number of Entrances');
          updateTable('cyclesPerDay','Cycles Per Day');
          updateTable('speakerSystem','Speaker System');
          updateTable('actionHistory','Action History');
          updateTable('controlAccss','Remote Control');
          updateTable('smartPhone','Smart Phone');
          updateTable('batteryBackup','Battery Backup');
          updateTable('solarPower','Solar Power');
          updateTable('loop','Loop');
          updateTable('accessories','Accessories');
        },
        products = [
            {id:'csw24ul', type: 'High-Traffic Swing Gate Operator', appType:[{ name: 'Commercial',value:true},{ name: 'Gated Community',value:true},{ name: 'Industrial',value:false},{ name: 'Apartment/Condo with Parking',value:false},{ name: 'Off-Street Parking',value:true},{ name: 'Rural/Ranch',value:true},{ name: 'Light Commercial',value:true},{ name: 'Single Family Residence',value:true}],
             entranceType:[{name:'Slide',value:false},{name:'Swing',value:true},{name:'Barrier',value:false},{name:'Overhead',value:false}],
             userNum:[{name:'Less than 25',value:true},{name:'Less than 90',value:true},{name:'Less than 200',value:true},{name:'Less than 1000',value:true},{name:'Less than 2000',value:true},{name:'More than 2000',value:true}],
             entranceNum:[{name:'1',value:null},{name:'2',value:null},{name:'3 - 4',value:null},{name:'more than 4',value:null}],
             cyclesPerDay:[{name:'Light (< 100/day)',value:true},{name:'Medium (< 300/day)',value:true},{name:'Heavy (< 800/day)',value:true},{name:'Continuous (800+/day)',value:true}],
             speakerSystem:[{name:'Yes',value:null},{name:'No',value:null}],
             actionHistory:[{name:'Not important',value:null},{name:'Yes, and I use POTS',value:null},{name:'Yes, and I use IP',value:null}],
             controlAccss:[{name:'Codes / PINs',value:null},{name:'Cards / Fobs / RFID Tags',value:null},{name:'Remotes',value:true},{name:'Credentialed Remotes',value:null}],
             smartPhone:[{name:'Yes',value:true},{name:'No',value:null}],
             batteryBackup:[{name:'Yes',value:true},{name:'No',value:null}],
             solarPower:[{name:'Yes',value:true},{name:'No',value:null}],
             loop:'LOOPDETLM',accessories:['LMRRUL','LMTBUL','LMWEKITU','L504AL','SP20W12V']},
...
//                console.log(productPackage,rankedPackage);
    ...
    if(is_iPad){
        ...
        $('.nav div#first a').css('font-weight',300);
        $('.nav div#first h2').css('font-weight',400);
        $('h1').css('font-weight',400);
        $('.nav div#last .right h2').css('font-weight',400);
        $('.nav div#first p').css('font-weight',300);
        $('<style>li span {font-weight: 500;} .nav div#last .right p span { font-weight: 500; }</style>').appendTo('head');
    }
});